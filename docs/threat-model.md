...
| The Mac app opens a connection of its own | The Swift shell uses no networking API | `scripts/check-deps.sh`, check g |
...
| The app is tampered with or loads foreign code | Hardened runtime, no entitlements, no dynamic libraries | `scripts/check-deps.sh`, check i. Signing and notarisation: not built (task 5.5) |
...
| Code signing and runtime | `scripts/check-deps.sh`, check i: hardened runtime on, no entitlements. The Developer ID signature and notarisation come with the release workflow (task 5.5) |
| Signed data updates | Not built. The Ed25519 check comes with the data-table updates (task 5.5) |
...
| Supply chain | `cargo deny check` in the check workflow; the crate allow-list in `scripts/check-deps.sh`. Actions are pinned to major versions, not to commits. The SBOM comes with the release workflow (task 5.5) |
...
- **Signed data updates, signing, notarisation and the SBOM.** Wait for the
  release workflow and data-table updates in task 5.5.
- **Sparkle's update check.** The one planned exception to local-only mode.
  Not built. When it is, `github.com` moves from planned to contacted in
  [data-flow.md](data-flow.md), and local-only mode needs a test that it
  stays off.
- **The secure text field in the injection matrix.** The nightly matrix runs
  ...
  instead.
- **Pinned actions.** GitHub Actions are pinned to major tags. Pinning to
  commit hashes is a small change to make before the first release.
