...
| A forged compatibility table is loaded | A downloaded table is checked against the Ed25519 key built into the core before it is parsed or saved; the saved copy is checked again at every start. Without a real key nothing is requested | `crates/aralo-core/tests/signed_data.rs` (eight tests). `crates/aralo-core/tests/data_update.rs`: `one_changed_byte_is_refused_and_nothing_is_saved`, `a_table_signed_by_another_key_is_refused`, `a_truncated_table_or_a_garbage_signature_is_refused`, `well_signed_garbage_is_refused_after_the_signature_check`, `a_cache_that_was_edited_or_torn_is_passed_over`, `the_committed_placeholder_key_means_no_download`. `scripts/verify-release.sh`, check f |
| An old signed table is replayed | Every table carries a signed `revision`; a download must be newer than the table in use, and a saved copy newer than the bundled one | `crates/aralo-core/tests/data_update.rs`: `an_older_signed_table_cannot_replace_a_newer_one`, `a_build_with_a_newer_table_than_the_cache_uses_its_own` |
| A table download is sent somewhere else | Only https on port 443 to github.com and GitHub's two asset hosts, never an IP address; redirects are followed by hand, five at most, each checked the same way | `crates/aralo-core/tests/data_update.rs`: `a_redirect_off_the_listed_hosts_is_not_followed` (another host, 127.0.0.1, another port, http, a relative path), `a_redirect_loop_stops`, `an_error_status_or_an_oversized_table_is_refused` |
| A table download leaves the Mac in local-only mode | Refused three times: the refresh runs only when Sparkle's check is allowed, the core refuses before any request, and the network guard refuses github.com | `data_update.rs`: `in_local_only_mode_nothing_is_requested`, `the_network_guard_refuses_github_in_local_only_mode_too`, `without_a_key_nothing_is_requested` |
...
| Signed data updates | `crates/aralo-core/tests/signed_data.rs`: a table signed by `scripts/sign-data.sh` is accepted; one changed byte, another key's signature, a malformed signature or no key at all is refused, and the bundled table stays in use. `scripts/verify-release.sh`, check f, on the release's `apps.toml.sig`. The download: `crates/aralo-core/tests/data_update.rs` (19 tests: signature, key, rollback, truncation, garbage, local-only, redirects, cache reload, atomic replace) and `crates/aralo-ffi/tests/data_tables.rs` (4 tests). `DataTablesTests.testWithoutADataTableKeyARefreshAsksNothing`. The key in `data/keys/data-tables.pub` is a placeholder until the first release, so today every refresh stops before a request |
...
- **Downloading signed data tables.** The download, the checks and the
  cache are built and tested against a fake GitHub. Not yet run for real: the
  public key in `data/keys/data-tables.pub` is a placeholder until the first
  release, so every build refuses before a request. The quirks table the
  plan names does not exist yet; only the compatibility table is covered.
