...
| 5.2 | Library location picker and move; iCloud and Dropbox smoke tests | Partial | 096241f | The move (copy, check, switch, the index carried), the bridge calls and the Settings window's Library tab landed in the commit that marks this row Partial. The smoke tests are `crates/aralo-core/tests/library_move.rs` and `crates/aralo-ffi/tests/library_move.rs`: iCloud Drive and Dropbox played in a temporary folder, with placeholders, eviction and download, delivery through a temporary file and a conflict copy after the move. Not coverable without real accounts on two Macs: the done-when check that two Macs converge within the provider's own sync time, and files marked dataless by a real iCloud Drive. Not run here: `make app` and `make test-swift`, because the disk was full. The Swift was type-checked against freshly generated bindings and passes SwiftLint strict |
| 5.3 | Section 9 tests, data-flow.md, threat-model.md | Partial | bc08d63 | Every section 9 row is mapped to a test in [threat-model.md](threat-model.md). Not coverable yet: P9 script approvals and P3 encrypted library (v1). Signing, runtime, signed data and the SBOM are covered by `scripts/verify-release.sh` and `signed_data.rs` since 5.5; the Developer ID and notarisation checks run once the release secrets are set |
| 5.4 | Full onboarding | Partial | a916a93 | Six screens: privacy, Accessibility, Input Monitoring, the library (whose snippets, Show in Finder, Import Snippets…), an AI offer that leaves AI off, and try-it. A returning user sees only the permission screens. Checked by `OnboardingTests` and the bridge test `the_first_run_can_tell_the_starter_snippets_from_the_users_own`. Not done: "a new user expands a first snippet in under 3 minutes" needs a timed run with new users. The library screen's "Move or Choose Folder…" opens Settings on its Library tab (5.2), where the folder is chosen or moved; "Set Up AI…" opens it on the AI tab |
| 5.5 | Release workflow: signing, notarisation, DMG, Sparkle | Partial | 71dba6a | Built: `scripts/release.sh` (universal build, Developer ID signing inside out, notarytool and stapler, DMG, appcast with EdDSA, signed data tables, CycloneDX SBOM, SHA256SUMS, unsigned-bundle hash), `scripts/verify-release.sh`, `.github/workflows/release.yml`, Sparkle 2.10.0 in the app (off in local-only mode, off without a real key), Ed25519 data-table check in `aralo-core` (`tests/signed_data.rs`). The dry run runs in CI and locally. Needs credentials: the Developer ID certificate, the notary API key, the Sparkle and data-table keys as secrets, and the data-table public key committed to `data/keys/data-tables.pub` ([releasing.md](releasing.md)). The app fetches signed tables (`aralo_core::data_update`, `Core.refresh_compat_table` on Sparkle's schedule, through the network guard, with a signed `revision` against rollback and an atomic cache; `tests/data_update.rs`, `aralo-ffi/tests/data_tables.rs`); not yet run for real until the data-table key is committed, and the quirks table does not exist yet. Done when not yet checked: a notarised DMG opening on a clean Mac without a warning, and an older build offering the update |
| 5.6 | Homebrew cask and documentation site | Partial | 15f3781 | Built: the cask template `packaging/homebrew/aralo.rb.in`, filled by each release, installed and uninstalled from a local tap in the release workflow's dry run; the mdBook site (`book.toml`, `docs/SUMMARY.md`) and `.github/workflows/docs.yml`. Needs: the tap repository and `HOMEBREW_TAP_TOKEN`, Pages set to GitHub Actions, and a signed release for `brew install --cask` to fetch |
