...
- The tap is suspended while any window of Aralo's own holds the keyboard, and a
  ...
  up.
- A snippet with `{{ai}}` blocks shows them in the same panel, after its form:
  the form's Enter reads Continue, and nothing is typed until the blocks are
  read. Each block's answer streams into the preview, where the model's text
  is marked and the rest is the snippet's own. Enter inserts, R asks again, E
  edits the answer, Stop keeps what has arrived and Escape cancels. A block
  no model answered says why, and shows the fallback that will go in. See
  [AI blocks in snippets](#ai-blocks-in-snippets).
...
`add`, `edit`, `remove`, `default`, `test`, `probe`, `models`, `detect`, `command`,
`write`). It
...
### AI blocks in snippets

A body asks a model for part of its text with
`{{ai: prompt | fallback: … | model: …}}`
([placeholders.md](format/placeholders.md#ai-blocks)). The evaluator asks no
model: `aralo-template` lists a body's blocks (`Resolved::ai_blocks`) and
renders what it is handed for each one as an `AiAnswer`, `Text` or `Fallback`,
pushed as literal text. It records where each block's text landed
(`Rendered::ai_spans`), so a panel can mark it and a test can prove that every
byte outside it is the body's.

**The session.** A body with a block always opens a `Session`. Its steps are
the form, then the context, then `SessionStep::Ai` with the blocks not yet
settled, then the plan. The kinds the snippet declared in `ai.context` are
asked for in the context step, with the kinds the body's own placeholders
read. They are kept apart from those: a declared `selection` is sent to the
model, but it does not expand a `{{selection}}` placeholder, which the format
does not expand yet. Only the declared kinds are asked for, and only when the
body has a block. `Session::block_request(index)` builds what a block sends:
the prompt, the profile and model (the block's `model:`, then `ai.model`), and
the text of each declared kind. `fillins` is the form's answers, one
`label: answer` per line. The body's text around the block is never sent. The
shell then settles each block with `answer_block(index, text)`, where the text
may be the model's answer or the user's edit of it, or with
`fall_back(index, reason)`. A block can be settled again until the last one
brings the plan. `preview_blocks(answers, drafts)` renders the expansion with
the text written so far.

**The run.** `AiSettings::run_block(request)` refuses when AI is off or there
is no profile. Otherwise it goes through the gateway as `Feature::Block` with
the declared kinds, from a `ContextSource` that answers only from what the
session collected. Local-only mode and the managed allow-list are the
gateway's to enforce, as for every feature. The answer streams through a
`BlockRun`, and `BlockRun::answer` fits it to its place: white space at the
ends and a code fence around the whole are removed. Every refusal and failure
is a reason to fall back, and the expansion carries it as an `AiFallback` or
`AiNothing` note. `AiSettings::answer_blocks(session)` settles every waiting
block without a panel. `aralo expand --ai` and `aralo type --ai` do the same
through the simulator, and without `--ai` a block falls back and says how to
ask.

**The Mac side.** `FormSession` in AraloKit drives the AI step as it drives
the form. It asks a `BlockRunner` for each block in turn. In the app that is
the one `AiProfiles`, opened the first time a block asks, so a snippet with no
block reads neither profiles.toml nor the keychain. The context comes from a
`SessionContext`, which reads only what the session names: the pasteboard,
the selection through Accessibility (never from a password field or while
secure input is on), the front app's name and its window's title. The block
state it shows (writing, written, failed and why), the edit, Regenerate, Stop
and the fallback are all tested with a fake runner in `FormSessionAITests`.
The editor's test field gets the same runner, so a draft's blocks can be tried
before it is saved.

### AI actions in the editor

The snippet editor has an AI menu beside Insert. It can draft a body, fix
spelling and grammar, make the text clearer or shorter, change its tone
(friendlier, more formal, more casual), translate it, or suggest variations.
`aralo-core`'s `Authoring` names each action, and `AiSettings::run_authoring`
runs one through the gateway as `Feature::Authoring` on the default profile.

**What is sent.** An action works on the selection in the body, or on the
whole body when nothing is selected. That text is the one piece of context,
declared as `Selection` and framed as data. A draft sends no text from the
editor: only the snippet's label and the note the user writes. The system
prompt says a snippet body is a template, and asks the model to keep every
placeholder and add none it was not asked for. Every refusal (nothing to work
on, nothing to draft from, no language, more than 100 KB, AI off, no profile)
comes before anything is sent.

**What comes back.** `versions(action, original, answer)` fits the answer to
the text it replaces, as a command's answer is fitted: the original's edges
and line endings, and no code fence around the whole. A draft is trimmed
instead. A variations answer is split at lines that hold only `%%%`, after
any fence around the whole is removed. `placeholder_changes(before, after)`
names the placeholders the answer dropped and the ones it added. Placeholders
that differ only in spacing count as the same. A placeholder a model writes is
one the snippet will expand, so a `{{clipboard}}` the model added is named
before the user takes it.

**The Mac side.** `AuthoringStore` in AraloKit is the sheet's model: the
answer as it streams, the versions to choose from, the draft with its word
diff and placeholder changes, Edit, Regenerate and Stop. It is tested with a
fake runner. The sheet puts the draft in with `TextReplacement.replace`, which
makes one `insertText(_:replacementRange:)` in its own undo group named after
the action. One ⌘Z in the editor takes it back, and Edit ▸ Undo says which
action it undoes. If the text under the sheet changed, nothing is replaced and
the sheet says why. The AI settings are opened the first time an action asks.
...
instead, so a beta tester can say what happened without anyone seeing what
they typed (plan 5.7). `aralo-core/src/counters.rs` keeps the counts and
`aralo-core/src/report.rs` turns them into text.

**What is counted.** Matches and sessions started from typing, snippets
picked from a list, text typed or pasted in, undos, buffer resets by reason,
insertions refused by reason, sessions cancelled, imports and what became of
their entries, outside changes and merged conflict copies. The shell adds
what only it sees: the event tap timed out, turned off for user input or
turned back on, a permission taken away, secure input coming on, and a
selection a command could not read. Beside the counts there is a histogram
of how long the core took to plan an expansion (under 1, 5, 10, 30 and
100 ms, and more), the apps expansions and undos went into, the model calls
made for commands, blocks and editor actions with their errors, and the last
20 errors by kind and minute.

**No content, by construction.** Every name a count is filed under comes
from an enum in `counters.rs`, so the only strings stored are ones written
there. Two things come from outside and are chosen to carry nothing typed: an
app is its bundle ID, and anything that does not look like one is filed as
`other`; a provider is its adapter kind and whether the endpoint is on this
machine, never a profile name or an address. An error is kept as its kind,
never its message, because a message can quote a path, an abbreviation or a
model's answer.

**Where they live.** One small `counters.json` in Aralo's state folder, the
folder that holds the index and `profiles.toml` (Application Support/Aralo
on the Mac). The library's `Core` and the `AiProfiles` open the same folder,
and `Counters::shared` hands both the one set of counts, so neither
overwrites the other. Counting is a short lock and no file. The counts are
written when a report is built, when the app quits, when the last holder
drops them, and on the shell's two-second permission timer through
`save_counters_if_due`, which writes only once a change has waited a
minute. Losing the file costs only the counts.

**The report.** `DiagnosticReport` puts together the facts the shell passes
(the app and system versions, both permissions, whether the tap runs,
secure input, pause), the library in outline (the kind of folder it is in
but never its path, how many snippets, groups and abbreviations, files that
did not read, the locale, whether the index is up), the AI settings in
outline (the switches, how many profiles, each one's adapter kind and
whether it is local, whether it has a key) and the counters. The menu bar's
Copy Diagnostics item, shown once a library is open, asks the core for it
and puts it on the pasteboard as plain text, for the tester to read and
paste into an issue. `aralo diagnostics <library>` prints the same report
from a terminal, without the facts only the app knows.

`crates/aralo-ffi/tests/diagnostics.rs` is the check. It marks every string
a user or a model could write: the library folder, a group, file names,
labels, abbreviations, bodies, the words typed around an abbreviation, a
form answer, the clipboard, a selection, a model's answers, profile names, a
header, a server's address and an API key. It drives all of it through the
bridge the way the app does, including a model call that fails, builds the
report, and fails if the mark, the key, the folder or the address appears
in it or in `counters.json`, while the counts it expects are there.
`DiagnosticsTests` in AraloKit does the same from Swift, and
`diagnostics_prints_counts_and_nothing_from_the_library` in the CLI's tests
covers the command.
...
| `AiProfiles`, AI blocks | `run_block(session, index) -> AiBlockRun`: the block's request is built from what the session holds. A refusal comes back before anything is sent, and is the shell's cue to `fall_back` | Async |
| `AiBlockRun` | `next() -> String?`, `cancel()`, `block()`, `profile()`, `model()`, `sent()`, `text()`, `answer() -> String?`: the answer fitted to its place, `cut_short()` | `next` async, the rest synchronous |
| `AiProfiles`, editor actions | `run_authoring(AiAuthoring, text) -> AiAuthoringRun`: `Draft { label, note }`, `Proofread`, `Clearer`, `Shorter`, `Friendlier`, `Formal`, `Casual`, `Translate { language }`, `Variations`. Refusals come back before anything is sent | Async |
| `AiAuthoringRun` | `next() -> String?`, `cancel()`, `profile()`, `model()`, `sent()`, `original()`, `text()`, `versions() -> [String]`: the answer fitted to the text it replaces, one per variation, `cut_short()` | `next` async, the rest synchronous |
| `Core`, commands | `commands() -> [AiCommand]`: the built-in ones, then the library's | Synchronous |
| `Core`, diagnostics | `record_shell_event(ShellEvent)`: `TapTimedOut`, `TapDisabledByUser`, `TapReenabled`, `PermissionRevoked`, `SecureInputOn` or `SelectionUnreadable`, cheap enough for the tap thread; `diagnostics_report(DiagnosticFacts, ai?) -> String`, where `DiagnosticFacts` holds the app and system versions and what the shell can tell of permissions, the tap, secure input and pause, and `ai` is the `AiProfiles` already open; `save_counters_if_due()`, `save_counters()`, `clear_counters()` ([local counters](#local-counters-and-the-diagnostics-report)) | Synchronous |
| `Engine`, commands | `command_target(app) -> CommandTarget`: `Ready { profile }` or `Refused { reason }`. Clears the typing buffer | Synchronous |
| Functions | `core_version()`, `excluded_app_presets()`, `placeholder_choices()`: what an editor's insert menu offers, `compat_apps(path?)`: the rows of a table, for the injection matrix, `ai_provider_presets()`, `diff_words(before, after)`, `authoring_label(action)`, `placeholder_changes(before, after)` | Synchronous |
...
| --- | --- | --- |
| Callbacks the shell implements | Optional `SecretStore` | Later. A session asks the shell for context by returning `SessionAction.Context`, and a block is asked again by running it again, so neither a `ContextProvider` nor a `regenerate` call is needed |
...
Where the data goes, and the complete list of hosts, is in
[data-flow.md](data-flow.md). What each promise defends against, and a row
by row map of the plan's security table to its tests, is in
...
| No expansion while secure input is on (P2) | The shell polls `IsSecureEventInputEnabled()`, resets, and names the app that holds it in the menu | `BridgeTests.testSecureInputGoingOnForgetsWhatWasTyped` turns real secure input on and runs the monitor the app runs. The engine side, `reset(SecureInput)`, is covered by `privacy.rs` |
...
| Keys only in the keychain (P13) | `SecretStore` on the login keychain. `profiles.toml` holds a `key_ref`, never a value, and refuses a key-shaped header or address. The CLI takes a key only from the environment or standard input ([profiles and keys](#profiles-and-keys)). A conformance report names the endpoint by host, says only whether a key was sent, and redacts anything key-shaped in an endpoint's message; `aralo conformance --key-env` sends a key for one run and saves it nowhere ([the conformance suite](#the-conformance-suite)) | `crates/aralo-core/tests/security.rs`: after every AI feature and every export format, the key is in no file. `crates/aralo-core/tests/ai_settings.rs`: `after_every_operation_the_key_is_only_in_the_store_and_the_auth_header`, with a canary key. `scripts/key-leak-scan.sh`: no key in any file a full test run wrote. `crates/aralo-core/tests/conformance.rs`: no key in any report, a key given for a run is in no file, and no committed report looks like it holds one |
| No Aralo servers (P7) | The app contacts only the endpoints the user saves ([data-flow.md](data-flow.md#every-host-aralo-can-contact)). The Mac shell uses no networking API | `crates/aralo-cli/tests/hosts.rs` scans the binary, the Swift sources and `data/` for hosts the page does not list. `scripts/check-deps.sh`: the offline-shell check |
| No logs | Aralo writes none. Crash reports are macOS's own | `scripts/check-deps.sh`: no log call in the Swift shell, no logging crate in the workspace |
| Hardened runtime | On, with no entitlements | `scripts/check-deps.sh` reads `apps/macos/project.yml` |
| No telemetry; local counters hold no content (P8) | Nothing is uploaded. Counts are filed under names from an enum, apps by bundle ID, providers by adapter kind, errors by kind and never message ([local counters](#local-counters-and-the-diagnostics-report)) | `crates/aralo-ffi/tests/diagnostics.rs`: `the_report_holds_no_snippet_context_typed_text_or_key`, and `DiagnosticsTests` in AraloKit |
