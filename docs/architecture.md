...
| --- | --- | --- |
| `Core` | `save_snippet`, `search`, `preview`, and `import`/`export` on the bridge | M2 |
...
`Core::import`, `Core::export` and `Core::search` exist in `aralo-core`
already and `aralo` drives them from the command line. They reach Swift when
the editor needs them, with the import report as a bridge type the app can
show ([ADR-0013](adr/0013-import-and-export.md)) and a search hit carrying the
characters that matched, so the list can highlight them without searching
again.
...
| --- | --- | --- |
| 3 | `aralo-ffi`, `aralo-cli` | Bridge for the M1 slice. The CLI also imports, exports, searches and expands |
| 2 | `aralo-core` | Open a library, build a plan, the compatibility table, in-memory simulator, import, export and search |
